...
void timeEncounter() {
  ...
  while(!(checkOne && checkTwo && sensorValue < 550)) {
    sensorValue = analogRead(sensorPin);
    if(sensorValue < 400) checkOne = true;
    if(sensorValue > 700) checkTwo = true;
    encounterTime++;
//    Serial.println(sensorValue);
  }
  lcd.clear();
  ...
  //For Relic Cave
    if((encounterTime > 96000 && encounterTime < 126000) || encounterTime > 131000) {
    while(true) {
      lcd.setCursor(0,1);
...
void loop() {
  ...
  walkUntilLessThan(400); //Walk until encounter starts
  timeEncounter();
  totalEncounters++;
  ...
  runAway();
  delay(5000); //Wait for walking screen
}
